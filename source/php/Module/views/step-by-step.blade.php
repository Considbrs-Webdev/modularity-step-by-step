        ...
        <div class="c-step-by-step-timeline__content">
            <{{ $componentElement }} class="{{ $class }}" {!! $attribute !!}>
                @foreach ($list as $section)
                    <{{ $sectionElement }}
                        class="{{ $baseClass }}__section c-step-by-step-timeline__section @if ($section['open_by_default']) c-step-by-step-timeline__section--open @endif">
                        <{{ $sectionHeadingElement }}
                            class="{{ $baseClass }}__button @if ($section['open_by_default']) {{ $baseClass }}__button--expanded @endif"
                            type="button" role="button" aria-label="{{ $section['heading'] }}"
                            ...
                            <span class="{{ $baseClass }}__button-wrapper" tabindex="-1">
                                ...
                                @icon([
                                    ...
                                    'size' => 'lg',
                                    'classList' => [$baseClass . '__icon', $baseClass . '__icon--add']
                                ])
                                @endicon
                                @icon([
                                    'icon' => 'fa-solid fa-minus',
                                    'size' => 'lg',
                                    'classList' => [$baseClass . '__icon', $baseClass . '__icon--remove']
                                ])
                                @endicon
                            </span>
                            </{{ $sectionHeadingElement }}>
                            <{{ $sectionContentElement }} class="{{ $baseClass }}__content"
